class tripleo::network::contrail::logrotate_config (
  ...
  $rotate_user_opts = 'su root root',
  $config           = '/etc/logrotate-crond.conf',
  $contrail_config  = '/etc/contrail-logrotate.conf',
  $copytruncate     = true,
  $delaycompress    = true,
  $compress         = true,
  $rotation         = 'daily',
...
# TODO: do edit config file if there is hiera item: contrail log path regex.
  if $step == 5 {
    ...
    exec { "add_openstack_section_to_new_config":
      command => "cat '${config}' >> '${contrail_config}' && mv -f '${contrail_config}' '${config}'",
